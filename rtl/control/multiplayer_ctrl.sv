module multiplayer_ctrl (
    ...
    input  logic       space_pressed,   // NOWE: wejście impulsu spacji z klawiatury
    ...
    output logic       launch_game      // NOWE: wyjście uruchomienia gry na obu monitorach
);
            ...
            case (tx_state)
                TX_IDLE: begin
                    if (!game_enable) begin
                        ...
                        // Handshake: Jeśli spacja wciśnięta w menu, wyślij token READY (0xD1)
                        if (space_pressed && !tx_sync_sent && !tx_busy) begin
                            tx_data      <= 8'hD1;
                            tx_start     <= 1'b1;
                            tx_sync_sent <= 1'b1;
                        end
                    end else begin
                        // Standardowe wysyłanie punktów podczas gry
    ...
    // --- ODBIÓR UART I LOGIKA HANDSHAKE ---
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            opp_score_hunds <= '0;
            opp_score_tens  <= '0;
            ...
            if (!game_enable) begin
                ...
                // Monitorowanie lokalnego wciśnięcia spacji
                if (space_pressed) begin
                    local_ready <= 1'b1;
                end
                
                // Monitorowanie odebrania gotowości od przeciwnika
                if (rx_ready && (rx_data == 8'hD1)) begin
                    remote_ready <= 1'b1;
                end
                
                // Wspólny start: Aktywacja sygnału gdy OBA układy zgłoszą gotowość
                if (local_ready && remote_ready) begin
                    launch_game <= 1'b1;
                end
            end else begin
                // W trakcie rozgrywki czyścimy flagi gotowości menu oraz odbieramy punkty
                local_ready  <= 1'b0;
                remote_ready <= 1'b0;
                launch_game  <= 1'b0;
...
module multiplayer_ctrl (
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (tx_state)
                TX_IDLE: begin
                    if ((game_enable && (current_my_score != prev_my_score)) || (game_over && !game_over_prev)) begin
                        prev_my_score <= current_my_score;
                        tx_data  <= {4'hA, my_score_hunds};
                        tx_start <= 1'b1;
                        tx_state <= TX_WAIT_H;
