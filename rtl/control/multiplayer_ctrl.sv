module multiplayer_ctrl (
    ...
    // Wykrywanie startu nowej gry
    logic game_enable_prev;
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) game_enable_prev <= 1'b0;
        else        game_enable_prev <= game_enable;
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // Resetujemy ubiegły wynik DOPIERO gdy startuje gra
            if (game_enable && !game_enable_prev) begin
                prev_my_score <= '0;
            end

            case (tx_state)
                TX_IDLE: begin
                    if (!game_enable && space_pressed && !tx_busy) begin
                        tx_data  <= 8'hD1;
                        tx_start <= 1'b1;
                    end 
                    else if ((current_my_score != prev_my_score) || (game_over && !game_over_prev)) begin
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (!game_enable) begin
                if (space_pressed) local_ready <= 1'b1;
                if (rx_ready && (rx_data == 8'hD1)) remote_ready <= 1'b1;
